...
# Tasks
* [x] Task 0. List all databases
Write a script that lists all databases in MongoDB.
```
khalfan@aisha:~/0x01$ cat 0-list_databases | mongo
...
bye
khalfan@aisha:~/0x01$
khalfan@aisha:~/0x01$ cat 1-use_or_create_database | mongo
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017
MongoDB server version: 3.6.3
...
```
khalfan@aisha:~/0x01$ cat 2-insert | mongo my_db
MongoDB shell version v3.6.3
...
* [x] Task 3. All documents

Write a script that lists all documents in the collection ``school``:

- The database name will be passed as option of ``mongo`` command
```
khalfan@aisha:~/0x01$ cat 3-all | mongo my_db
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017/my_db
MongoDB server version: 3.6.3
...
* [x] Task 4. All matches

Write a script that lists all documents with ``name="Holberton school"`` in the collection ``school``:

- The database name will be passed as option of ``mongo`` command
```
khalfan@aisha:~/0x01$ cat 4-match | mongo my_db
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017/my_db
MongoDB server version: 3.6.3
{ "_id" : ObjectId("5a8fad532b69437b63252406"), "name" : "Holberton school" }
...
- The database name will be passed as option of ``mongo`` command
```
khalfan@aisha:~/0x01$ cat 5-count | mongo my_db
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017/my_db
MongoDB server version: 3.6.3
1
...
- The database name will be passed as option of mongo command
```
khalfan@aisha:~/0x01$ cat 6-update | mongo my_db
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017/my_db
MongoDB server version: 3.6.3
...
* [x] Task 7. Delete by match

Write a script that deletes all documents with ``name="Holberton school"`` in the collection ``school``:

- The database name will be passed as option of mongo command
```
khalfan@aisha:~/0x01$ cat 7-delete | mongo my_db
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017/my_db
MongoDB server version: 3.6.3
...
* [x] Task 8. List all documents in Python
...
```
khalfan@aisha:~/0x01$ cat 8-main.py
#!/usr/bin/env python3
""" 8-main """
...
* [x] Task 9. Insert a document in Python
...
- Returns the new ``_id``
```
khalfan@aisha:~/0x01$ cat 9-main.py
#!/usr/bin/env python3
""" 9-main """
from pymongo import MongoClient
list_all = __import__('8-all').list_all
...
* [x] Task 10. Change school topics
...
```
khalfan@aisha:~/0x01$ cat 10-main.py
#!/usr/bin/env python3
""" 10-main """
from pymongo import MongoClient
list_all = __import__('8-all').list_all
...
* [x] Task 11. Where can I learn Python?
...
- Display (same as the example):
	* [x] first line: ``x logs`` where ``x`` is the number of documents in this collection
	* [x] second line: ``Methods``:
	* [x] 5 lines with the number of documents with the method = ["GET", "POST", "PUT", "PATCH", "DELETE"] in this order (see example below - warning: it’s a tabulation before each line)
	* [x]one line with the number of documents with:
	 	* [x] ``method=GET``
	 	* [x] ``path=/status``
...
```
khalfan@aisha:~/0x01$ curl -o dump.zip -s "https://s3.amazonaws.com/intranet-projects-files/holbertonschool-webstack/411/dump.zip"
khalfan@aisha:~/0x01$ 
khalfan@aisha:~/0x01$ unzip dump.zip
Archive:  dump.zip
   creating: dump/
   creating: dump/logs/
...
khalfan@aisha:~/0x01$ 
khalfan@aisha:~/0x01$ mongorestore dump
...
khalfan@aisha:~/0x01$ 
khalfan@aisha:~/0x01$ ./12-log_stats.py 
94778 logs
Methods:
    method GET: 93842
    method POST: 229
...
* [x] Task 13. Regex filter
Write a script that lists all documents with name starting by Holberton in the collection school:

- The database name will be passed as option of mongo command
```
khalfan@aisha:~/0x01$ cat 100-find | mongo my_db
MongoDB shell version v3.6.3
connecting to: mongodb://127.0.0.1:27017/my_db
MongoDB server version: 3.6.3
...
```
khalfan@aisha:~/0x01$ cat 101-main.py
#!/usr/bin/env python3
""" 101-main """
from pymongo import MongoClient
list_all = __import__('8-all').list_all
...
khalfan@aisha:~/0x01$ 
khalfan@aisha:~/0x01$ ./101-main.py
...
* [x] Task 15. Log stats - new version
Improve 12-log_stats.py by adding the top 10 of the most present IPs in the collection nginx of the database logs:

- The IPs top must be sorted (like the example below)
```
khalfan@aisha:~/0x01$ ./102-log_stats.py 
94778 logs
Methods:
    method GET: 93842
    method POST: 229
